...
- Ensure Python 3.8+ is installed and accessible in `PATH` (the template supports every version listed in `pyproject.toml`).
...
- Users can manually create a shortcut pointing to `%LOCALAPPDATA%\Programs\yourapp-starter-<version>\tic-tac-toe-starter.vbs`.
...
- Update `wheel-builder.bat` (shortcut creation block) and rebuild. Deleting `%TEMP%\create_yourapp_shortcut.vbs` between runs helps avoid stale scripts.
